...
{}
void Jugador::agregarObjeto(unique_ptr<Objetos> obj){
    inventario.push_back(std::move(obj));
}
void Jugador::mostrarInventario() const{
    using namespace std;
    //Se puede hacer más bonito, pero no voy a implementar apenas más objetos
...
bool Jugador::usarObjeto(int indice, Personajes* objetivo){
    ...
    if (indice < 0 || indice >= static_cast<int>(inventario.size())) {
        cout << "Índice de objeto no válido.\n";
        return false;
    }
    // 2. Transferir propiedad del objeto fuera del vector
    unique_ptr<Objetos> obj = move(inventario[indice]);
